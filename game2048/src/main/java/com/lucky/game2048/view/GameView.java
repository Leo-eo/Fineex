package com.lucky.game2048.view;
...
import com.lucky.game2048.activity.GameActivity;
import com.lucky.game2048.bean.GameItem;
...
public class GameView extends GridLayout implements OnTouchListener {
    ...
    private void initGameMatrix() {
        ...
        setColumnCount(mGameLines);//列
        setRowCount(mGameLines);//行
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (isMoved()) {
                    addRandomNum();
                    // 修改显示分数
                    GameActivity.getGameActivity().setScore(Config.SCROE, 0);
                }
                checkCompleted();
                break;
            default:
    ...
    private void judgeDirection(int offsetX, int offsetY) {
        ...
        /*else if (flagSuper) { // 启动超级用户权限来添加自定义数字
			...
			et.setHint("只能是2的N次幂(N∈[1~10])");
			et.setInputType(InputType.TYPE_CLASS_NUMBER);
			builder.setTitle("修改下一次出现的数字").setView(et)
    ...
    private void checkCompleted() {
        ...
        if (result == 0) {
            if (Config.SCROE > mHighScore) {
                ...
                editor.apply();
                GameActivity.getGameActivity().setScore(Config.SCROE, 1);
                Config.SCROE = 0;
            }
            AlertDialog.Builder builder = new AlertDialog.Builder(getContext());
            ...
            AlertDialog.Builder builder = new AlertDialog.Builder(getContext());
            builder.setTitle("任务完成").setPositiveButton("重来", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface arg0, int arg1) {
                    ...
                    if (mTarget == 1024) {
                        ...
                        mTarget = 8192;
                        GameActivity.getGameActivity().setGoal(8192);
                    } else if (mTarget == 8192) {
                        editor.putInt(Config.KEY_GAME_GOAL, 16384);
                        mTarget = 16384;
                        GameActivity.getGameActivity().setGoal(16384);
                    } else if (mTarget == 16384) {
                        editor.putInt(Config.KEY_GAME_GOAL, 32768);
                        mTarget = 32768;
                        GameActivity.getGameActivity().setGoal(32768);
                    } else {
                        editor.putInt(Config.KEY_GAME_GOAL, 32768);
                        mTarget = 32768;
                        GameActivity.getGameActivity().setGoal(32768);
